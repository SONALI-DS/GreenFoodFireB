...
public class FoodCategoryAdapter extends RecyclerView.Adapter<FoodCategoryAdapter.CategoryHolder> {
    ...
    public void onBindViewHolder(@NonNull CategoryHolder holder, int position) {
        ...
        if (position == selectedItem){
            // Make card selected
            holder.cardView.setOutlineSpotShadowColor(context.getColor(R.color.orange_500));
            holder.cardView.setOutlineAmbientShadowColor(context.getColor(R.color.orange_500));
            holder.cardView.setStrokeWidth(2);
            holder.title.setTextColor(context.getColor(R.color.orange_500));
            holder.image.setColorFilter(ContextCompat.getColor(context,R.color.orange_500), PorterDuff.Mode.SRC_IN);
        }else {
            // Make card inactive
